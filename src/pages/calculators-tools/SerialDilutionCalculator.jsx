...
import {
  ...
  Button,
  Input,
} from "@/components/ui"; // Adjust this import path as needed

// Create fallback components in case motion fails
...
function SerialDilutionCalculatorContent() {
  ...
  const results = useMemo(() => {
    ...
    if (
      [C_stock_raw, C_init_raw, DF_raw, V_final_raw, N_raw].some(
        (n) => isNaN(n) || n <= 0
      )
    ) {
    ...
    if (C_stock === null || C_init === null)
      errs.push("Provide a valid molar mass for unit conversion.");
    if (C_init > C_stock)
      errs.push("Initial concentration cannot exceed stock concentration.");
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-4">
      ...
      <Card className="bg-white/5 backdrop-blur-md">
        <CardContent className="grid grid-cols-1 gap-4 p-6 md:grid-cols-2 lg:grid-cols-3">
          <Input
            placeholder="Stock solution name"
            value={stockName}
          ...
          <div className="flex items-center gap-2">
            <Input
              type="number"
              className="flex-1"
              placeholder="Initial conc."
              value={initConc}
              onChange={(e) => setInitConc(e.target.value)}
            />
          ...
          <Input
            type="number"
            placeholder="Number of dilutions"
            value={numDilutions}
            onChange={(e) => setNumDilutions(e.target.value)}
          />
          <Input
            type="number"
            placeholder="Replicates (optional)"
            value={replicates}
            min={1}
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {results.map((row) => (
                <tr
                  key={row.step}
                  className={row.step % 2 === 0 ? "bg-white/5" : "bg-white/10"}
                >
